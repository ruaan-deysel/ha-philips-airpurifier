...
During setup, Home Assistant waits for the first payload from that stream and then keeps the stream open for later updates. The integration does not perform periodic status reads. If the observe stream raises an error, ends, or stays silent for a conservative idle window, the integration marks the device unavailable and reconnects with bounded backoff using only the public `philips-airctrl` client API. The idle recovery is passive: it waits for the existing push stream to go stale instead of probing the purifier with extra CoAP requests. Structured diagnostic events are written to `philips_airpurifier_debug.jsonl` inside the Home Assistant config directory so setup, observe, reconnect, control, availability, and idle-timeout events can be inspected even when standard Home Assistant logs are not written to the Samba share.
