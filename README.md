...
During first-time setup, Home Assistant waits for the first payload from that stream and then keeps the stream open for later updates. The integration does not perform periodic status reads, and it does not close and reopen the stream just because values are quiet. Some Philips firmware appears to treat any new status observation as a scarce resource, so quiet periods are left alone. If a device is offline during Home Assistant startup but cached status exists, the integration loads the entities as unavailable and reconnects in the background instead of forcing Home Assistant into a rapid setup-retry loop. If the observe stream raises an error or ends, the integration marks the device unavailable and reconnects with bounded backoff using only the public `philips-airctrl` client API. A reconnect is considered successful once the public client has been recreated and a single observe stream has been opened; it does not cancel and reopen that stream while waiting for a fresh payload. Structured diagnostic events are written to `philips_airpurifier_debug.jsonl` inside the Home Assistant config directory so setup, observe, reconnect, control, and availability events can be inspected even when standard Home Assistant logs are not written to the Samba share.
