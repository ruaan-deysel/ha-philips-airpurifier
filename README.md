...
Current versions use `philips-airctrl` 1.1.0 or newer. Device status is refreshed with a bounded public CoAP status snapshot using `observe=False`, then a long-lived public CoAP observe stream is kept open for push updates. If the observe stream stays quiet, the integration refreshes another cancelled snapshot at the device's advertised max-age, usually 60 seconds. This avoids rapid polling while keeping Home Assistant from showing stale power or air-quality values as live.

During first-time setup, Home Assistant fetches one fresh snapshot and then starts the observe stream for later updates. The integration does not close and reopen the observe stream just because values are quiet. Some Philips firmware appears to treat status observations as a scarce resource, so the fallback snapshot rate is bounded by the device max-age instead of using a fast polling loop. If a device is offline during Home Assistant startup but cached status exists, the integration loads the cached data internally but keeps the entities unavailable until a fresh snapshot or observe payload arrives. A single slow periodic snapshot is treated as a missed refresh; repeated snapshot failures, observe stream errors, or observe stream endings mark the device unavailable and reconnect with bounded backoff using only the public `philips-airctrl` client API. Structured diagnostic events are written to `philips_airpurifier_debug.jsonl` inside the Home Assistant config directory so setup, observe, snapshot, reconnect, control, and availability events can be inspected even when standard Home Assistant logs are not written to the Samba share.
...
| ------------ | ------------------------------------------ | ----------------------- |
| **AC0650**   | AC0650/10                                  | Compact Air Purifiers   |
